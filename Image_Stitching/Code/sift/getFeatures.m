...
% 设置初始sigma
init_sigma = 1.6;
% 降维得到的每个组（octave）都有若干图像（interval）
intvls = 3;
s = intvls;
k = 2^(1/s);
sigma = ones(1,s+3);
...
% 为得到更多特征点，将图像放大至原来的两倍
input_img = imresize(input_img,2);
% 假设原图像已有sigma=0.5的高斯平滑，对扩大两倍后的图像做一次高斯平滑
input_img = gaussian(input_img,sqrt(init_sigma^2-0.5^2*4));
% 上一个octave的长宽均为下一个octave的两倍，按照下式，顶层octave图像的长度和宽度最小值在8像素左右
